...
const DeviceWidget = ({deviceNo=null,type=null,icon}) => {
      ...
      <View style={widgetStyle}>
        <View style={deviceWidgetStyle.topPart}>
          {(type=="Window")?(Device==0? DefaultWindow:DarkThemeWindow):(
            ...
            :<Icon name={icon} type='font-awesome-5' color='#4392F1'/>)}
          {Device != null? 
          <Switch onValueChange={()=>{(type=="Lamp")?switchLight(deviceNo,Device,dispatch):switchWindow(deviceNo,Device,dispatch);
          switchDevice(type);}} value={(Device == 0 )? false:true} thumbColor={(Device == 1 || Device == 180 )? "#5BEC5B" : "#f4f3f4"} trackColor={{ false: "#767577", true: "#C2FFC2" }}/>:null}
        </View>
        <View style={deviceWidgetStyle.bottomPart}>
            <Text style={(Device == 1 || Device == 180) ? deviceWidgetOn.room : deviceWidgetStyle.room}>{(type=="Lamp")?"Light":((type=="Window")?"Window":"Add a device")}</Text>
        </View>
      </View>
    );
  }
